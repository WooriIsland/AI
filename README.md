...
- 본 프로젝트에서 제공된 RTX 4090 환경 기준으로 OpenChat 7B, Zephyr 7B 모델들을 테스트해볼 수 있었으나, 해당 모델이 프롬프트에 대한 이해도나 JSON 방식으로 출력하는 성능이 떨어짐. 또한 OpenChat의 경우 지연 시간이 매우 길어 이러한 오픈소스 모델들로 구현하는데는 큰 한계점
