...
- 하지만 LLaVA는 사진에 대한 사실 자체만을 묘사하는데 그치고, 본 '우리가족섬' 서비스에서 표현하고자 하는 날짜/시간, 인물, 배경 정보를 포함하고 재치있거나 감성적인 형태로 사진을 요약하는데는 한계
...
### ✔️ Openchat /GPT-4-1106-Preview / Langchain을 활용한 AI 챗봇
...
- 본 프로젝트에서 제공된 RTX 4090 환경 기준으로 OpenChat 7B, Zephyr 7B 모델들을 테스트해볼 수 있었으나, LLM 모델이 프롬프트에 대한 이해도나 JSON 방식으로 출력하는 성능이 떨어져 오픈소스 모델들로 구현하는데 한계
